    ...
    <div className="min-h-screen bg-zinc-950 text-white p-6 md:p-10">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col gap-5">
          <h2 className="text-4xl font-extrabold tracking-tight">Recent Notes</h2>
          ...
          {Task.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              {Task.map((itm, idx) => {
                return (
                  <div key={idx} className="relative bg-white text-black rounded-2xl p-4 min-h-44 flex flex-col justify-between shadow-md hover:shadow-xl transition-shadow">
                    <div className="flex flex-col gap-1 pr-6">
                      <h3 className="font-bold text-base leading-snug break-words">{itm.text}</h3>
                      <p className="text-sm text-zinc-500 break-words">{itm.input}</p>
                    </div>
                    <button
